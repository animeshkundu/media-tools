...
| --- | --- | --- | --- |
| Firefox | Ubuntu | [Firefox E2E](../.github/workflows/e2e.yml) installs the built add-on in the CI-provisioned Firefox and drives its real `moz-extension://` app page through geckodriver. It exercises WAV cut and export, MP3 export, MP3 input, join, change speed, download signatures, and no-egress resource inspection. | Release-tested as an installed extension with the extension-page CSP enforced. MP3 input is capability-scoped on every run: supported configurations must decode and re-export, while unsupported configurations must show the clear decoder error without a partial download. |
...
Selenium-Manager-provisioned Firefox. Each Ubuntu CI run records its own capability branch: support
requires a completed decode and re-export, while an unsupported result requires the clear user-facing
decoder error, an intact app, and no partial download.
