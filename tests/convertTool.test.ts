...
vi.mock('@/lib/tools/convert/convert', () => ({
  startConversion: () => ({ cancel: () => undefined, result: Promise.resolve(new Blob()) }),
}));

const { decodeFileForConvert } = await import('../entrypoints/app/ConvertTool');

describe('ConvertTool worker-routed decode', () => {
  it('happy path: resolves with correct PCM, duration, and sampleRate from startDecodeFile', async () => {
    ...
    const result = await decodeFileForConvert(file, onProgress);
  ...
  it('rejects when startDecodeFile rejects with the 256 MB pre-decode cap error', async () => {
    ...
    await expect(decodeFileForConvert(file, () => undefined)).rejects.toThrow(
      /256\s*MB/,
    );
  ...
  it('rejects when startDecodeFile enforces the 64 MB input cap synchronously', async () => {
    ...
    await expect(decodeFileForConvert(oversized, () => undefined)).rejects.toThrow(
      /smaller than 64 MB/,
    );
  });
