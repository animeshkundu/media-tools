...
export const MAX_MULTITRACK_ASSETS = 128;
...
function validateClip(clip: AudioClip, asset: AudioAsset): void {
  ...
  assertFiniteRange(clip.playbackRate, 0.25, 4, 'Clip speed');
  ...
  if (clip.trimStart + clip.duration * clip.playbackRate > asset.duration + Number.EPSILON) {
...
export function validateTimelineState(state: TimelineState): void {
  ...
  if (assets.length > MAX_MULTITRACK_ASSETS) {
    throw new Error(`A project supports at most ${MAX_MULTITRACK_ASSETS} media assets.`);
  }
