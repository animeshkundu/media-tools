...
export interface CanvasTimelineProps {
  ...
  readonly onSkim?: (time: number) => void;
  readonly skimmingEnabled?: boolean;
}

function roleColor(role: AudioTrack['role']): string {
  if (role === 'dialogue') return '#63b7ff';
  if (role === 'music') return '#a78bfa';
  return '#fbbf24';
}
...
export function CanvasTimeline({
        ...
        for (const clip of track.clips) {
          ...
          if (pyramid && asset && clipWidth > 4) {
            ...
            for (let pixel = startX; pixel < endX; pixel += 1) {
              ...
              const sourceSample =
                (clip.trimStart +
                  (timelineTime - clip.startTime) * clip.playbackRate) *
                asset.sampleRate;
              const bin = Math.max(
                0,
                Math.min(
  ...
  function handlePointerMove(event: PointerEvent<HTMLCanvasElement>): void {
    ...
    if (!skimmingEnabled || !onSkim) return;
    const now = performance.now();
    if (now - lastSkimAtRef.current < 80) return;
    lastSkimAtRef.current = now;
    onSkim(pointerPosition(event).time);
  }
  ...
  function handleKeyDown(event: KeyboardEvent<HTMLCanvasElement>): void {
    ...
    const edit = event.altKey
      ...
      : event.ctrlKey || event.metaKey
        ? 'trim-end'
        : 'move';
    const edited = nudgeClipWithKeyboard(
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-white/10 bg-black/20">
        ...
        >
          <div className="relative" style={{ width: contentWidth, height: contentHeight }}>
            <canvas
              ...
              aria-label="Multitrack waveform timeline. Select clips, drag their body to move, or drag gold edges to trim. Keyboard commands can move and trim the selected clip."
      ...
      <div className="sr-only" aria-label="Timeline clips">
        <p id="timeline-keyboard-help">
          Use Left and Right Arrow to nudge the selected clip by 0.01 seconds, or hold Shift for
          0.1 seconds. Hold Alt to trim the clip start. Hold Control or Command to trim the clip
          end. Home and End move the playhead.
        </p>
