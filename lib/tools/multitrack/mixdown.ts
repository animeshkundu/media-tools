...
function clipEnvelope(clip: AudioClip, localTime: number): number {
...
function createTrackSampler(
  ...
  return (frame) => {
    ...
    for (const { clip } of active) {
      ...
      const sourcePosition = (clip.trimStart + localTime) * pcm.sampleRate;
      ...
      const gain = clip.gain * track.volume * clipEnvelope(clip, localTime);
