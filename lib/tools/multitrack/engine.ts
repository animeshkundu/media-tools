...
import { clipEnvelopeGain } from './mixdown';
import type {
  AudioAssetId,
  AudioClip,
  FadeCurve,
  ...
  return previous.tracks.some((track, trackIndex) => {
    ...
    return track.clips.some((clip, clipIndex) => {
      ...
      return (
        ...
        clip.playbackRate !== nextClip.playbackRate
      );
    });
  });
}

export type ScheduledClipWindow = {
  readonly clipLocalTime: number;
  readonly duration: number;
  readonly timelineDelay: number;
};

export function scheduledClipWindow(
  clip: AudioClip,
  position: number,
  maximumDuration?: number,
): ScheduledClipWindow | undefined {
  const clipEnd = clip.startTime + clip.duration;
  if (clipEnd <= position) return undefined;
  const timelineDelay = Math.max(0, clip.startTime - position);
  if (maximumDuration !== undefined && timelineDelay >= maximumDuration) {
    return undefined;
  }
  ...
  const availableWindow =
    maximumDuration === undefined
      ...
      : maximumDuration - timelineDelay;
  const duration = Math.min(clip.duration - clipLocalTime, availableWindow);
  return duration > 0 ? { clipLocalTime, duration, timelineDelay } : undefined;
}

export function fadeAutomationMethod(curve: FadeCurve): 'linear' | 'exponential' {
  return curve === 'linear' ? 'linear' : 'exponential';
}

export function scheduledFadeInRampEnd(
  clip: AudioClip,
  clipLocalTime: number,
  endLocalTime: number,
): number | undefined {
  return clip.fadeIn > clipLocalTime
    ? Math.min(clip.fadeIn, endLocalTime)
    : undefined;
}
...
export class MultitrackAudioEngine {
  ...
  #scheduleFrom(position: number, maximumDuration?: number): void {
    ...
    for (const track of this.#state.tracks) {
      ...
      for (const clip of track.clips) {
        const window = scheduledClipWindow(clip, position, maximumDuration);
        if (!window) continue;
        const { clipLocalTime, duration: sourceDuration, timelineDelay } = window;
        ...
        source.start(
          scheduleTime + timelineDelay,
          clip.trimStart + clipLocalTime * clip.playbackRate,
          sourceDuration * clip.playbackRate,
        );
      }
    }
  }
  ...
  #scheduleClipGain(
    ...
    const method = fadeAutomationMethod(clip.fadeCurve);
    const fadeGain = (time: number): number =>
      clip.gain * clipEnvelopeGain(clip, time);
    const setValue = (value: number, time: number): void => {
      parameter.setValueAtTime(
        method === 'exponential' ? Math.max(0.001, value) : value,
        time,
      );
    };
    const rampToValue = (value: number, time: number): void => {
      if (method === 'linear') {
        parameter.linearRampToValueAtTime(value, time);
      } else {
        parameter.exponentialRampToValueAtTime(Math.max(0.001, value), time);
      }
    };
    setValue(fadeGain(clipLocalTime), startTime);
    const fadeInRampEnd = scheduledFadeInRampEnd(
      clip,
      clipLocalTime,
      endLocalTime,
    );
    if (fadeInRampEnd !== undefined) {
      rampToValue(
        fadeGain(fadeInRampEnd),
        startTime + fadeInRampEnd - clipLocalTime,
      );
    }
    ...
    if (clip.fadeOut > 0 && fadeOutStart < endLocalTime) {
      ...
      setValue(fadeGain(rampStart), startTime + rampStart - clipLocalTime);
      rampToValue(fadeGain(endLocalTime), startTime + duration);
    }
  }
